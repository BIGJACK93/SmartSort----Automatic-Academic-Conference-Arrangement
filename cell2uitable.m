...
for i = 1:total
    ...
    if incell{i}==1, incell{i}='Boolean'; end
    if incell{i}==0, stat=stat+1; incell{i}='Boolean'; 
        unselected{end+1} = incell{i-nrow}; incell{i-nrow} = ''; end
end
for i = 1:ncol
    if strcmp(incell{1, i},'')
        removal_col(end+1) = i;
    end
end
incell(:, removal_col) = [];
incell = regexprep(incell, ',', '');
disp 'Unselected papers are listed as below:'
disp(unselected)
